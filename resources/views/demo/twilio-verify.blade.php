...
<head>
    ...
    <title>Enter the SMS code</title>
    <style>
...
<body>
    <form method="POST" action="{{ route('demo.twilio.verify.submit') }}">
        @csrf
        <h1>Enter the SMS code</h1>
        <p class="lead">We sent a code via <strong>{{ $provider }}</strong> to your phone. Enter it to verify.</p>

        @if ($errors->any())<div class="err">{{ $errors->first() }}</div>@endif

        <input name="code" inputmode="numeric" autocomplete="one-time-code" placeholder="••••••" autofocus data-testid="twilio-code">
        <button type="submit">Verify</button>
        <p class="alt"><a href="/demo/twilio">Use a different number</a></p>
    </form>
</body>
</html>
